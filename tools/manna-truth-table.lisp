...
(defun checked-static-fixture-p (root layout topology advantage2 advantage360)
  ...
  (unless (= 73 (count-prefixed-lines topology "  (position "))
    (error "Manna topology must contain the complete 73-position device union."))
  ...
  (unless (and (= 68 (count-prefixed-lines advantage2 "  (place "))
    ...
    (error "Manna device coverage must classify all 73 positions (A2: 68/5; A360: 72/1)."))
  (unless (and (search "(position direct-hyper)" topology)
               (fixture-placement-p advantage2 "direct-hyper" "RWIN" "rmet")
               (fixture-placement-p advantage360 "direct-hyper" "RWIN" "rmet")
               (not (search "(binding direct-hyper" layout)))
    (error "Manna direct rmet source must remain a placed pass-through without an invented binding."))
  t)
