...
if [ "$#" -ne 2 ] && [ "$#" -ne 3 ]; then
    echo "usage: $0 PATH-TO-kanata-1.12.0.tar.gz FROZEN-MANNA-ROOT [GENERATED-CONFIG]" >&2
    exit 2
fi
...
if [ ! -f "$selected_config" ]; then
    echo "kanata oracle: selected config is not a regular file: $selected_config" >&2
    exit 1
fi
...
cp "$selected_config" "$source_directory/cfg_samples/ivory-key-manna.kbd"
