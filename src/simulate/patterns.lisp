...
KIND is one of :EVENT, :SEQUENCE, :ALL, :EITHER, :AND, :DURATION,
...
(defstruct (pattern-match-context
             (:constructor make-pattern-match-context
                 (&key events start-index anchor-index)))
...
(defun pattern-position-matches-p (wanted actual)
  (or (eq wanted :any)
      (equal wanted actual)
      (and (consp wanted)
...
(defun event-pattern-matches-p (pattern event)
...
they belong under AND/ALL instead of SEQUENCE/WITHIN in normalized IR."
  (when (eq (event-pattern-kind pattern) :event)
    (context-event-indices context
                           (lambda (event) (event-pattern-matches-p pattern event)))))
   ...
   (context-event-indices
    ...
    (lambda (event)
      (and (eq (timed-event-kind event) :down)
           (pattern-position-matches-p position (timed-event-position event)))))
...
(defun collect-deadline-patterns (pattern)
  ...
  (cond
    ...
    ((member (event-pattern-kind pattern) '(:sequence :all :either :and :within :repeat)
             :test #'eq)
