...
| 7 — full Manna Cadet migration | In progress, not migration-complete | The frozen 52-key/eight-context table, 158 explicit `NoSymbol` cells, six shared navigation routes, four Advantage360 local F-key routes, both device coverage declarations, 29-output function patch, realization vocabulary, deterministic carrier proposal, four direct case/Greek/Top holders, and all 16 source-selected tap-hold structures are transcribed. The generated frozen diff closes the native ledgers for all 68 Advantage2 and 72 Advantage360 primary `defsrc` positions while retaining reviewed C7/game differences and the typed-unreachable `<LSGT>` disposition. A hash-pinned [Kanata 1.12 source-state-machine oracle](kanata-1.12-oracle.md) proves delayed pending input, repeated/multi-key routing, five modifier pairs, and final-owner function lifetime. The buffered reference path has a per-plan barrier with immutable physical evidence, closed provenance, ordered repeated direct intervals, reverse-UP pairing, all equal-deadline eligible owners, shared held-effect reference counts, and atomic unequal-deadline refusal. Its direct route remains context-free named-key only; C7/game/unmapped domains, overlays, stateful/latch-sensitive and timed routes still refuse. No Manna profile selects this policy and no Kanata backend lowers it. The generated XKB carrier/state result remains a separate 51-override libxkbcommon sub-contract. Backend/profile selection, function activation/arbitration, client/live input delivery, Manna equivalence, and the full generated Kanata-to-XKB differential remain unproved. |
