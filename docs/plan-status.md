...
| 7 — full Manna Cadet migration | Complete; generated, not deployed | The frozen 52-key/eight-context table, 158 explicit `NoSymbol` cells, complete 68/72 native-route ledgers, reviewed C7/game differences, and typed-unreachable `<LSGT>` disposition are closed by the deterministic truth-table report. The selected profile covers four direct holders, all sixteen tap-holds, five modifiers, the function layer, selector carriers, A360 local keys, 29 source-backed carrier allocations, and one explicit stable `/dev/input/by-id/` endpoint and distinct virtual-output identity per device. Its per-plan dispatch barrier has immutable physical evidence, ordered repeated intervals, reverse-UP pairing, equal/unequal-deadline owners, deterministic peer commitment, and shared held-effect reference counts. Hash-pinned Kanata 1.12 tests cover the selected scheduler and generated A2/A360 artifacts; installed Kanata accepts both. Generated XKB contains 51 reviewed overrides, selector carriers, and distinct Control/Mod1/Mod2/Mod3/Mod4 mappings; libxkbcommon verifies all eight contexts, selector state, and effective unconsumed modifiers, and the Kanata-to-XKB AD01 differential passes. Normal project compilation now yields two exact artifacts, a relocatable source map, and 29 concrete provenance-bearing allocations; each Kanata artifact names only its selected physical device and a targetable output instead of accepting ambient defaults. No abstract layout source embeds XKB or Kanata syntax. Live installation/input proof belongs exclusively to Phase 8. |
| 8 — controlled integration | Live deployment accepted; final closeout pending | The [basedbox integration record](integration/2026-08-23-basedbox-phase-8.md) records the authorized A360 activation, pinned Kanata 1.12 managed service, build hashes, validator results, modifier repair, observed desktop behavior, and an untouched footswitch service. The authorized operator waived a disruptive rollback drill because the prior configurations are version-controlled and proven portable across machines. Publication and clean-worktree gates remain before the project may claim the whole plan complete. |
...
The detailed [cross-machine completion handoff](integration/2026-08-17-cross-machine-completion-handoff.md)
records the staged-only source snapshot, portable validation and publication
gates, Phase 8 authorization/rollback transaction, final audit, and the
subsequent plan-only McCLIM task.
